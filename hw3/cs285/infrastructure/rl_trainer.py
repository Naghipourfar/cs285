...
class RL_Trainer(object):
    ...
    def run_sac_training_loop(self, n_iter, collect_policy, eval_policy):
        ...
        for itr in range(n_iter):
            ...
            if itr == 0:
                ...
                paths, envsteps_this_batch = utils.sample_random_trajectories(self.env, use_batchsize,
                                                                              self.params['ep_len'])
                train_video_paths = None
    ...
    def perform_dqn_logging(self, all_logs):
        ...
        if self.mean_episode_reward > -5000:
            ...
            logs["Train_AverageReturn"] = -5000

        print("mean reward (100 episodes) %f" % self.mean_episode_reward)
        if self.best_mean_episode_reward > -5000:
            logs["Train_BestReturn"] = np.mean(self.best_mean_episode_reward)
        else:
            logs["Train_BestReturn"] = -5000
    ...
    def perform_sac_logging(self, itr, stats, eval_policy, train_video_paths, all_logs):
        ...
        # collect eval trajectories, for logging
        print("\nCollecting data for eval...")
        eval_paths, eval_envsteps_this_batch = utils.eval_trajectories(self.env, eval_policy,
                                                                       self.params['eval_batch_size'],
                                                                       self.params['ep_len'])
