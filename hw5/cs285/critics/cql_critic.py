...
class CQLCritic(BaseCritic):
    ...
    def dqn_loss(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        q_t_values = torch.gather(qa_t_values, 1, ac_na.unsqueeze(1)).squeeze(1)  # proper select Q(s, a)
        qa_tp1_values = self.q_net_target(next_ob_no)

        if self.double_q:
            next_actions = self.q_net(next_ob_no).argmax(dim=1)
            q_tp1 = torch.gather(qa_tp1_values, 1, next_actions.unsqueeze(1)).squeeze(1)  # select Q(s', a')
        else:
    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        info['CQL Loss'] = ptu.to_numpy(cql_loss).mean()
