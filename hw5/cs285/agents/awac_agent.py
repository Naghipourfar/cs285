...
class AWACAgent(DQNAgent):
    ...
    def get_qvals(self, critic, obs, action):
        ...
        q_value = torch.gather(critic.qa_values(obs, return_tensor=True), 1, action.long().unsqueeze(1)).squeeze(1)
        return q_value
    ...
    def estimate_advantage(self, ob_no, ac_na, re_n, next_ob_no, terminal_n, n_actions=10):
        ...
        action_dist = self.awac_actor.forward(ob_no)
        ...
        if self.agent_params['discrete']:
            ...
            q_sas = self.actor.critic.qa_values(ob_no, return_tensor=True).float().to(action_probs.device)
        else:
            # Mohsen: All environments in this HW are discrete so this scope is never being executed
            # for _ in range(n_actions):
            #     actions_per_state = action_dist.rsample((n_actions,))  # (n_actions, batch_size, ac_dim)
            #     qsas = self.actor.critic(ob_no)
            raise Exception('Action distribution is continuous! :D')

        v_pi = (q_sas * action_probs).sum(dim=-1, keepdims=True)

        # TODO Calculate Q-Values
        q_vals = self.get_qvals(self.actor.critic, ob_no, ac_na).unsqueeze(1)
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        if (self.t > self.learning_starts
            ...
            self.running_rnd_rew_std = self.rnd_gamma * expl_bonus.std() + (
                    1 - self.rnd_gamma) * self.running_rnd_rew_std
