    ...
    def estimate_advantage(self, obs: np.ndarray, rews_list: np.ndarray, q_values: np.ndarray, terminals: np.ndarray):
        ...
        if self.nn_baseline:
            ...
            if self.gae_lambda is not None:
                ...
                # print(obs.shape, sum([len(r) for r in rews_list]), rews.shape, q_values.shape, terminals.shape)
                # exit()
                ...
                for i in reversed(range(batch_size)):
                    ...
                    ## 0 otherwise.
                    if terminals[i] == 0:
                        advantages[i] = rews[i] + self.gamma * values[i + 1] - values[
                            i] + self.gae_lambda * self.gamma * advantages[i + 1]
                    if terminals[i] == 1:
                        advantages[i] = rews[i]
